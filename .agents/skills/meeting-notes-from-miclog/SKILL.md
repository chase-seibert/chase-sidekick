...
Generate copy/paste-ready meeting notes from `memory/miclog.txt` and nearby calendar context. The transcript file is generated by [miclog](https://github.com/chase-seibert/miclog). Also write a copy of the summary to a new section at the top of @local/miclog-summaries.md.
...
Return only Markdown notes suitable for pasting into Confluence, email, or Slack. Do not write files, post messages, send email, or update Confluence unless the user explicitly asks. Also write a copy of the summary to a new section at the top of @local/miclog-summaries.md.
